...
| `loading`    | `boolean` | `false` | 加载中；禁止点击并加 `v-btn-loading`；非 `icon` 变体时额外插入 `v-btn__loading` 指示；`icon` 变体不插入该元素（由插槽内容自行表达加载态）；不强制同步 `disabled` prop |
...
3. `loading`：根节点加 `v-btn-loading` 并禁用交互；仅当 `variant !== 'icon'` 时插入 `v-btn__loading` 元素
