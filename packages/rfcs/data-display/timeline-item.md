...
> 状态：**草案** | 优先级：**P2** | 分类：**Data Display 数据展示**
...
- **包**：`@opentiny/vue-next`
- **导出**：`TimelineItem`
- **scene-theme 类名**：`sc-timeline-item`（规划，scene-theme 尚未实现）
...
> 待细化：补充 TimelineItem 专属 named slots。
...
2. 根元素挂载 `sc-timeline-item` + `st-*` 状态类
3. 通过 hook 处理 DOM，state 驱动 UI
4. 销毁时清理监听与引用

> 待细化：TimelineItem 交互流程。
...
- Custom Element：`tvp-timeline-item`
